...
app.onError((err, c) => {
  if (err instanceof UserFacingError) return c.json({ error: err.message }, err.status as ContentfulStatusCode);
  console.error(err);
...
const explainInput = (q: Record<string, string | undefined>) => ({ url: q.url, owner: q.owner, repo: q.repo });
...
const WEB_ORIGINS = (process.env.WEB_ORIGIN ?? "http://localhost:3100,http://127.0.0.1:3100").split(",");
function assertLocalApp(c: { req: { header: (n: string) => string | undefined } }) {
  const origin = c.req.header("origin");
  if (!origin || !WEB_ORIGINS.includes(origin)) throw new UserFacingError("This action is only allowed from the app itself.", 403);
}
...
app.post("/explain/reports/:id/clone", async (c) => {
  ...
  const body = await c.req.json().catch(() => ({}));
  return c.json(await startClone(r, { base: String(body.base ?? ""), name: String(body.name ?? ""), branch: body.branch, shallow: body.shallow === true }), 202);
});
...
app.get("/explain/static/:id/*", async (c) => {
  ...
  return c.body(f.body as ArrayBuffer);
});
...
serve({ fetch: app.fetch, port: config.port }, (info) => {
  console.log(`AI-Repo-Assistant API on http://localhost:${info.port} (LLM: ${activeProvider()})`);
});
