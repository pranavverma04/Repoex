...
export function cleanReadme(readme, name = "") {
  ...
  const starter =
    /bootstrapped with \[?`?create-|This template provides a minimal setup|Getting Started with Create React App/i.test(
      text,
    );
  for (const para of starter ? [] : text.split(/\n\s*\n/)) {
    ...
    const plain = p
      ...
      .replace(/\s+/g, " ");
    if (
      /badge|shields\.io|npm version|build status|bootstrapped with|this template|getting started|start editing|auto-updates|learn more about|deploy(ed)? on vercel|create-next-app|create react app|readme\.md|docs?:\s|next\/font|this project uses/i.test(
        plain,
      )
    )
      continue;
    ...
    if (
      /^(click|run|install|clone|to (get|install|run|start)|first,|please|note:|see |check |read |follow|open|download|copy)\b/i.test(
        plain,
      ) ||
      /:$/.test(plain)
    )
    ...
    const describes =
      /\b(is|are) (a|an|the)\b|\blets you\b|\bhelps?\b|\ballows?\b|\bprovides?\b|\benables?\b|\bdesigned\b|\bbuilt (for|to|with)\b|\b(library|framework|tool|toolkit|app|application|website|platform|service|api|client|server|plugin|extension|engine|cli)\b/i;
  ...
  const sec = text.match(
    /^#{1,3}\s*(?:✨\s*)?(?:key\s+)?(?:features|highlights|why\b[^\n]*|what it does)[^\n]*\n([\s\S]*?)(?=\n#{1,3}\s|$)/im,
  );
  if (sec) {
...
export function templateOverview(r, readme) {
  ...
  const runtime = (cat) =>
    r.stack.groups
      ...
      ?.items.filter((i) => !i.dev && i.name.toLowerCase().split(" ")[0] !== self)
      .map((i) => i.name) ?? [];
  ...
  const what = kindText.length
    ? listWords(kindText.map((k, i) => (i === 0 ? `a ${k}` : k)).map((s) => s.replace(/^a ([aeiou])/, "an $1")))
    : "a software project";
  const oneLiner =
  ...
  const plain = [];
  plain.push(
    `${r.repo.name} is ${what}${kinds.length === 1 && kinds[0] === "library" ? " that other code can import" : ""}.`,
  );
  ...
  if (r.mock.web?.pages.length)
    plain.push(
      `It has ${r.mock.web.pages.length} page${r.mock.web.pages.length === 1 ? "" : "s"} you can visit, starting at ${r.mock.web.pages[0].route}.`,
    );
  if (r.mock.api?.endpoints.length)
    plain.push(
      `Other programs talk to it through ${r.mock.api.endpoints.length} API endpoint${r.mock.api.endpoints.length === 1 ? "" : "s"}.`,
    );
  if (r.mock.cli)
    plain.push(
      `You run it in a terminal as \`${r.mock.cli.binName}\`${r.mock.cli.commands.length ? `, with ${r.mock.cli.commands.length} command${r.mock.cli.commands.length === 1 ? "" : "s"}` : ""}.`,
    );
  if (r.mock.library) plain.push(`You add it to your own project with \`${r.mock.library.install}\`.`);

  const tech = [];
  const allLangs = r.stack.languages
    .filter((l) => l.pct >= 0.1)
    .slice(0, 4)
  ...
  for (const g of r.stack.groups.slice(0, 6)) {
    tech.push(
      `${g.category}: ${g.items
        .slice(0, 6)
        .map((i) => `${i.version ? `${i.name} ${i.version}` : i.name}${i.dev ? " (dev)" : ""}`)
        .join(", ")}.`,
    );
  }
  ...
  if (r.structure.entryPoints.length)
    tech.push(
      `Entry points: ${r.structure.entryPoints
        .slice(0, 4)
        .map((e) => e.path)
        .join(", ")}.`,
    );
  const topFolders = r.structure.folders
    .filter(
      (f) =>
        f.path !== "(root)" &&
        !/^(tests?|__tests__|spec|test-d|e2e|docs?|examples?|\.github|media|assets|public)$/i.test(f.path),
    )
    .slice(0, 3);
  if (topFolders.length)
    tech.push(`Most code lives in ${topFolders.map((f) => `${f.path}/ (${f.files} files)`).join(", ")}.`);
...
const SCHEMA = {
  ...
  properties: {
    ...
    plain: {
      type: "string",
      description: "3-5 sentences for a non-programmer: what it does, who it's for, how someone uses it.",
    },
    technical: {
      type: "string",
      description: "4-7 sentences for a developer: architecture, main modules, data flow, notable libraries.",
    },
    audience: { type: "string", description: "One sentence: who would use or work on this." },
    highlights: {
      type: "array",
      items: { type: "string" },
      description: "Up to 6 concrete capabilities, each under 15 words.",
    },
    workSummary: {
      type: "string",
      description: "2-4 sentences on what work has been done recently, based on the commit summary and highlights.",
    },
    endpointExamples: {
      type: "array",
      description:
        "For up to 8 of the listed endpoints: a realistic example JSON response body, consistent with any response code given.",
      items: {
        type: "object",
...
function factSheet(r, readme) {
  ...
  const lines = [
    ...
    `Top folders: ${r.structure.folders
      .slice(0, 10)
      .map((f) => `${f.path} (${f.files} files, ${f.purpose})`)
      .join("; ")}`,
    `Entry points: ${r.structure.entryPoints.map((e) => e.path).join(", ")}`,
    `Work summary: ${r.work.summary}`,
    `Recent notable commits: ${r.work.months
      .slice(0, 4)
      .flatMap((m) => m.highlights.slice(0, 3).map((h) => `[${h.type}] ${h.message}`))
      .join(" | ")}`,
  ];
  if (mock.web)
    lines.push(
      `Web pages: ${mock.web.pages.map((p) => `${p.route} (headings: ${p.headings.slice(0, 3).join(" / ") || "none"})`).join("; ")}`,
    );
  if (mock.api) {
    lines.push(`API (${mock.api.framework}) endpoints:`);
    for (const e of mock.api.endpoints.slice(0, 16)) {
      lines.push(
        `- ${e.method} ${e.path}${e.bodyFields.length ? ` body: ${e.bodyFields.join(", ")}` : ""}${e.responseFromCode ? ` returns: ${e.responseFromCode.slice(0, 300).replace(/\s+/g, " ")}` : ""}`,
      );
    }
  }
  if (mock.cli)
    lines.push(
      `CLI \`${mock.cli.binName}\` commands: ${mock.cli.commands.map((c) => `${c.usage} (${c.description ?? ""})`).join("; ")}; options: ${mock.cli.options.map((o) => o.flag).join(", ")}`,
    );
...
export async function enrich(r, readme) {
  ...
  try {
    ...
    if (r.mock.cli && out.cliExamples?.length) {
      for (const ex of out.cliExamples.slice(0, 3))
        r.mock.cli.session.push({ command: ex.command.replace(/^\$\s*/, ""), output: ex.output, aiWritten: true });
    }
