...
import { fetchMeta, fetchUserRepos, searchUserRepos } from "./source.js";
...
async function probe(url, source) {
  try {
    const res = await fetch(url, {
      ...
      headers: { "User-Agent": "Mozilla/5.0 ai-repo-assistant" },
    });
...
export async function findLiveSites(report) {
  ...
  if (home && /^https?:\/\//.test(home) && !/github\.com\/[^/]+\/[^/]+\/?$/.test(home))
    candidates.push([home, "homepage"]);
  try {
    const meta = await fetchMeta({ owner: report.repo.owner, name: report.repo.name });
    if (meta.hasPages) {
      const owner = report.repo.owner.toLowerCase();
      const pages =
        report.repo.name.toLowerCase() === `${owner}.github.io`
          ? `https://${owner}.github.io/`
          : `https://${owner}.github.io/${report.repo.name}/`;
      if (!candidates.some(([u]) => u.replace(/\/$/, "") === pages.replace(/\/$/, "")))
        candidates.push([pages, "github-pages"]);
    }
  } catch {
    // metadata is a nice-to-have here
  }
  const sites = (await Promise.all(candidates.map(([u, s]) => probe(u, s)))).filter((x) => x !== null);
 ...
 */
export async function staticFile(report, rawPath) {
