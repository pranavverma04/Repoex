...
import type { CommitInfo, CommitType, Contributor, MonthWork, RepoReport } from "@ara/shared";
import type { GhContributor, RawCommit } from "./source.js";

const CONVENTIONAL: Record<string, CommitType> = {
  feat: "feature", feature: "feature", add: "feature",
  fix: "fix", bugfix: "fix", hotfix: "fix",
  refactor: "refactor", perf: "refactor", style: "refactor",
  docs: "docs", doc: "docs",
  test: "tests", tests: "tests",
  chore: "maintenance", build: "maintenance", ci: "maintenance", deps: "maintenance", release: "maintenance", revert: "maintenance",
};

/** gitmoji.dev prefixes, used by many repos instead of conventional commits */
const GITMOJI: [string[], CommitType][] = [
  [["✨", "🎉", "🥚", "🚩"], "feature"],
  [["🐛", "🚑", "🩹", "🔒", "🥅", "🚨"], "fix"],
  [["♻", "🎨", "⚡", "🔥", "💄", "🚚", "🏷", "🗑", "💫", "🧑‍💻", "🦺", "🏗"], "refactor"],
  [["📝", "💡", "✏", "📄", "🌐", "💬"], "docs"],
...
export function classifyCommit(subject: string, files: string[]): CommitType {
  ...
  if (/^(revert|bump|release|v?\d+\.\d+\.\d+(-[\w.]+)?$|upgrade|update (dependencies|deps)|chore|require (node|python|go)|drop (support|node)|meta tweaks?$)/.test(l) || /\bdependabot\b|renovate/.test(l)) return "maintenance";
  if (/^(update|upgrade|bump|pin|unpin)\b.*\b(dependencies|deps|requirements|lockfile|versions?)\b/.test(l)) return "maintenance";
  if (/\b(fix(e[sd])?|bug|crash|resolve[sd]?|patch|hotfix|correct|repair|broken|issue #?\d+|regression)\b/.test(l)) return "fix";
  if (/\b(refactor|clean ?up|rename|restructure|simplif|reorganiz|move|tidy|perf|optimi[sz])/.test(l)) return "refactor";
  if (/\b(test|tests|spec|coverage)\b/.test(l)) return "tests";
  if (/\b(docs?|readme|typos?|comments?|documentation|changelog)\b/.test(l) || /^(document|clarify|explain|describe|mention)\b/.test(l)) return "docs";
  if (/^(add|adds|added|implement|introduce|create|support|new|enable|allow|initial)\b/.test(l) || /\b(feature|feat)\b/.test(l)) return "feature";
  if (files.length && files.every((f) => /\.(md|mdx|rst|txt)$/i.test(f) || /^docs?\//.test(f))) return "docs";
  if (files.length && files.every((f) => /(^|\/)(tests?|__tests__|spec)\/|\.(test|spec)\.\w+$|_test\.go$|test_\w+\.py$/.test(f))) return "tests";
  if (/^(preserve|respect|harden|guard|prevent|ensure|avoid|reject|enforce|strip|normalize|sanitize|validate|don'?t|do not|never|stop|gracefully|ignore|handle|restore|keep|throw|catch|retry|escape)\b/.test(l)) return "fix";
  if (/^(update|improve|change|tweaks?|minor|various|small|make|use|set|adjust|allow|remove|drop|delete|unify|skip|return|treat|run|type|merge|replace|convert|extract|split|bump|polish|upgrade|migrate|switch|refine|reduce|speed)\b/.test(l)) return "refactor";
  return "other";
}

const TYPE_ORDER: CommitType[] = ["feature", "fix", "refactor", "docs", "tests", "maintenance", "other"];
const TYPE_WORD: Record<CommitType, string> = {
...
const IMPORTANCE: Record<CommitType, number> = { feature: 6, fix: 5, refactor: 3, tests: 2, docs: 2, other: 1, maintenance: 0 };
...
function areaOf(file: string): string {
  ...
  if (["apps", "packages", "services", "libs", "src"].includes(parts[0]) && parts.length > 2) return `${parts[0]}/${parts[1]}`;
  return parts[0];
}
...
export function analyzeHistory(
  commits: RawCommit[],
  shallow: boolean,
  ghContributors: GhContributor[],
  releases: RepoReport["work"]["releases"],
  totalCommits: number | null,
): RepoReport["work"] {
  const infos: CommitInfo[] = commits.map((c) => ({
    sha: c.sha,
    message: c.subject,
    author: c.author,
    date: c.date,
  ...
  const isMerge = (c: CommitInfo) => /^merge (pull request|branch|remote)/i.test(c.message);
  ...
  const months: MonthWork[] = [...monthMap.entries()]
    .sort((a, b) => b[0].localeCompare(a[0]))
    .slice(0, 18)
    .map(([month, list]) => {
      const bt: Partial<Record<CommitType, number>> = {};
  ...
  const people = new Map<string, { name: string; email: string; commits: number }>();
  ...
  const byName = new Map<string, { name: string; email: string; commits: number }>();
  ...
  const contributors: Contributor[] = [...byName.values()]
    .sort((a, b) => b.commits - a.commits)
    .slice(0, 12)
    .map((p) => {
      const noreply = p.email.match(/^(?:\d+\+)?([\w-]+)@users\.noreply\.github\.com$/i)?.[1];
      const gh = (noreply && ghByLogin.get(noreply.toLowerCase())) || ghByLogin.get(p.name.toLowerCase().replace(/\s+/g, ""));
      return {
        name: p.name,
  ...
  const areaMap = new Map<string, Map<CommitType, number>>();
  ...
  const areas = [...areaMap.entries()]
    .map(([path, m]) => {
      ...
      const main = [...m.entries()].filter(([t]) => t !== "maintenance" && t !== "other").sort((a, b) => b[1] - a[1])[0]?.[0] ?? "other";
      return { path, commits: total, mainType: main };
    })
    .sort((a, b) => b.commits - a.commits)
    .slice(0, 10);
  const hotspots = [...fileMap.entries()]
    .filter(([f]) => !/(^|\/)(package-lock\.json|pnpm-lock\.yaml|yarn\.lock|poetry\.lock|Cargo\.lock|go\.sum|CHANGELOG\.md)$/i.test(f))
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
  ...
  if (infos.length) {
    ...
    const pct = (n: number) => `${Math.round((n / infos.length) * 100)}%`;
    ...
    if (top.length) {
      ...
      parts.push(heavyMaint ? `${pct(maint)} are routine maintenance (releases, dependency bumps); the rest went mostly into ${list}.` : `Most of the work went into ${list}.`);
    }
    const busiest = months.slice().sort((a, b) => b.count - a.count)[0];
    if (busiest && months.length > 1) parts.push(`The busiest month was ${monthName(busiest.month)} with ${busiest.count} commits.`);
