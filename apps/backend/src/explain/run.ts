...
import type { MockOutput, RepoReport, RunStep } from "@ara/shared";
...
const IGNORED_ENV = new Set(["NODE_ENV", "HOME", "PATH", "PWD", "USER", "SHELL", "TERM", "CI", "DEBUG", "TZ", "LANG", "TMPDIR", "NEXT_RUNTIME", "VERCEL", "GITHUB_ACTIONS", "FORCE_COLOR", "NO_COLOR"]);

function readmeSnippets(readme: string | null): { heading: string; code: string }[] {
  if (!readme) return [];
  const out: { heading: string; code: string }[] = [];
  let heading = "";
  let inRunSection = false;
  for (const m of readme.matchAll(/^(#{1,4})\s+(.+)$|```([\w-]*)\n([\s\S]*?)```/gm)) {
    if (m[2]) {
      heading = m[2].replace(/[`*_[\]]/g, "").replace(/\(.*?\)/g, "").trim();
...
export function buildRun(input: RunInput): RepoReport["run"] {
  const steps: RunStep[] = [];
  ...
  if (root?.scripts) for (const [name, command] of Object.entries(root.scripts)) scripts.push({ name, command: String(command) });

  const installs: string[] = [];
  if (root) installs.push(pm === "pnpm" ? "pnpm install" : pm === "Yarn" ? "yarn" : pm === "Bun" ? "bun install" : "npm install");
  ...
  else if (has(/^requirements\.txt$/)) installs.push("python -m venv .venv && source .venv/bin/activate", "pip install -r requirements.txt");
  else if (has(/^(pyproject\.toml|setup\.py)$/)) installs.push("python -m venv .venv && source .venv/bin/activate", "pip install -e .");
  ...
  if (installs.length) {
    ...
    steps.push({ title: "Install dependencies", commands: installs, note: [nodeVer, pyVer ? `Needs Python ${pyVer}.` : null].filter(Boolean).join(" ") || null });
  }
  ...
  const envFile = input.paths.find((p) => /(^|\/)\.env\.(example|sample|template|local\.example)$/.test(p) && p.split("/").length <= 3);
  const envVars = new Map<string, string>();
  for (const [f, t] of input.files) {
    if (/(^|\/)\.env\.(example|sample|template|local\.example)$/.test(f)) {
      for (const m of t.matchAll(/^\s*(?:export\s+)?([A-Z][A-Z0-9_]{2,})\s*=/gm)) if (!envVars.has(m[1])) envVars.set(m[1], f);
    }
  }
  for (const [f, t] of input.files) {
    ...
    for (const re of ENV_PATTERNS) {
      re.lastIndex = 0;
      for (const m of t.matchAll(re)) if (!IGNORED_ENV.has(m[1]) && !envVars.has(m[1]) && envVars.size < 30) envVars.set(m[1], f);
    }
  }
  if (envFile) {
    steps.push({ title: "Set environment variables", commands: [`cp ${envFile} ${envFile.replace(/\.(example|sample|template)$/, "").replace(/\.local\.example$/, ".local")}`], note: "Fill in the values it lists before starting." });
  } else if (envVars.size && isApp) {
    steps.push({ title: "Set environment variables", commands: [...envVars.keys()].slice(0, 6).map((k) => `export ${k}=…`), note: "The code reads these. Not all may be required." });
  }
  ...
  const hasFile = (re: RegExp) => input.paths.find((p) => re.test(p));
  if (!start.length && !isApp) {
    if (input.mock.cli) start.push(`${input.mock.cli.binName} --help`);
    else if (input.mock.library?.usage) {
      steps.push({ title: "Use it in your code", commands: [input.mock.library.install], note: "See the usage example under Mock output." });
    }
  }
  if (!start.length && isApp) {
    ...
    const streamlit = input.deps.has("streamlit") && [...input.files.entries()].find(([f, t]) => /\.py$/.test(f) && /import streamlit/.test(t));
    ...
    else if (hasFile(/^[\w-]+\/__main__\.py$/)) start.push(`python -m ${hasFile(/^[\w-]+\/__main__\.py$/)!.split("/")[0]}`);
    else if (hasFile(/^main\.go$/)) start.push("go run .");
    else if (hasFile(/^cmd\/[^/]+\/main\.go$/)) start.push(`go run ./${hasFile(/^cmd\/[^/]+\/main\.go$/)!.replace(/\/main\.go$/, "")}`);
  ...
  if (compose) steps.push({ title: "Or run everything with Docker", commands: ["docker compose up --build"], note: null });
  else if (hasFile(/^Dockerfile$/)) steps.push({ title: "Or run it in Docker", commands: [`docker build -t ${input.name.toLowerCase()} .`, `docker run --rm -it ${input.port ? `-p ${input.port}:${input.port} ` : ""}${input.name.toLowerCase()}`], note: null });
