...
export function buildTree(paths, maxChildren = 60, maxDepth = 6) {
  ...
  const toNode = (m, depth) => {
    ...
    if (depth < maxDepth && budget > 0) {
      ...
      for (const f of sortedFiles.slice(0, room - kids.length))
        kids.push({ name: f, path: m.path ? `${m.path}/${f}` : f, type: "file" });
    }
    return {
      name: m.name,
      path: m.path,
      ...
      files: m.count,
      children: kids,
      more: total - kids.length || undefined,
    };
  };
...
function folderPurpose(name, files) {
...
export function describeStructure(paths, pkgMain) {
  ...
  const folders = [...byTop.entries()]
    .map(([top, files]) => ({
      path: top,
      purpose: folderPurpose(top, files),
      files: files.length,
      share: files.length / total,
    }))
  ...
  if (rootFiles)
    folders.push({ path: "(root)", purpose: "Config and top-level files", files: rootFiles, share: rootFiles / total });
  ...
  for (const m of pkgMain) {
    ...
    const src =
      ...
      paths.find((p) =>
        ["src/", "source/", "lib/", ""].some(
          (d) =>
            /\.(ts|tsx|mts|js|mjs)$/.test(p) &&
            p.replace(/\.(ts|tsx|mts|js|mjs)$/, "") ===
              (base.includes("/") ? base.replace(/^([^/]*\/)?/, (x) => x) : d + base),
        ),
      );
    pushEntry(
      src ?? full,
      src && src !== full ? `source of "${m.field}" (${m.value}) in ${m.path}` : `"${m.field}" in ${m.path}`,
    );
  }
  for (const [re, why] of ENTRY_CANDIDATES) {
    for (const p of paths) {
      const rel = p
        .replace(/^(apps|packages|services)\/[^/]+\//, "")
        .replace(/^(backend|frontend|server|client|web|api)\//, "");
      if (
        re.test(rel) &&
        p.split("/").length <= 5 &&
        !/(^|\/)(tests?|__tests__|spec|examples?|docs?|scripts)\//.test(p)
      )
        pushEntry(p, why);
    }
  }
