...
import type { CliCommand, CliOption, Endpoint, MockOutput, ProjectKind, SiteTheme, WebPage } from "@ara/shared";
...
function resolveAsset(src: string, fromFile: string, s: SourceFiles): string | null {
  ...
  if (clean.startsWith("/")) {
    ...
    hit = s.paths.find((p) => p.endsWith(`public${clean}`) || p.endsWith(`static${clean}`)) ?? s.paths.find((p) => p === clean.slice(1));
  } else {
...
function pageImages(file: string, text: string, s: SourceFiles): string[] {
  const out: string[] = [];
  const imported = new Map<string, string>();
  for (const m of text.matchAll(/import\s+(\w+)\s+from\s+["']([^"']+\.(?:png|jpe?g|gif|svg|webp|avif))["']/gi)) imported.set(m[1], m[2]);
  for (const m of text.matchAll(/<(?:img|Image)\b[^>]*?\bsrc=(?:["']([^"']+)["']|\{\s*["'`]([^"'`]+)["'`]\s*\}|\{\s*(\w+)\s*\})/g)) {
...
function detectTheme(s: SourceFiles): SiteTheme | null {
  ...
  for (const [, t] of css) for (const m of t.matchAll(/(--[\w-]+)\s*:\s*([^;}]+)[;}]/g)) if (!vars.has(m[1])) vars.set(m[1], m[2].trim());
  ...
  const color = (v: string) => {
    const t = resolve(v.trim().replace(/!important/, "").trim());
  ...
  for (const [, t] of css) {
    for (const body of t.replace(/\/\*[\s\S]*?\*\//g, "").matchAll(/(?<=^|\})\s*(?:html|body)\s*(?:,[^{]*)?\{([^}]*)\}/g)) {
  ...
  for (const [, t] of tw) {
    accent ??= t.match(/(?:primary|brand|accent)\s*:\s*(?:\{[^}]*?DEFAULT\s*:\s*)?["'](#[0-9a-f]{3,8})["']/i)?.[1] ?? null;
  ...
  const logoPath =
    s.paths.find((p) => /(^|\/)(public|static|assets|src\/assets|images?|img)\/(?:[\w-]+\/)?[\w.-]*(logo|brand|mark)[\w.-]*\.(svg|png|webp|jpe?g)$/i.test(p) && !isTestPath(p)) ??
  ...
  const darkClass = [...s.files.entries()].some(([f, t]) => /layout\.(tsx|jsx)$|index\.html$/.test(f) && /className=["'][^"']*\bdark\b|class=["'][^"']*\bdark\b/.test(t));
  const bareHsl = background?.match(/^hsla?\([^,\s]+[\s,]+[\d.]+%[\s,]+(\d+(?:\.\d+)?)%/) ?? background?.match(/^oklch\(\s*(\d*\.?\d+)/)?.map((x, i) => (i === 1 ? String(Number(x) * 100) : x));
...
const isTestPath = (p: string) => /(^|\/)(tests?|__tests__|spec|e2e|fixtures|examples?|docs?|benchmarks?)\//i.test(p) || /\.(test|spec)\.\w+$/.test(p);
...
function allText(text: string, re: RegExp, limit: number, min = 1): string[] {
  const out: string[] = [];
 ...
 */
function withImports(file: string, text: string, files: Map<string, string>, depth = 0): string {
  if (depth > 1) return text;
  let out = text;
  const dir = file.split("/").slice(0, -1).join("/");
  const rootOf = file.match(/^(.*?\/)?(src\/|app\/|pages\/)/)?.[1] ?? "";
  for (const m of text.matchAll(/import\s+(?:(\w+)|\{([^}]+)\})\s+from\s+["']([^"']+)["']/g)) {
    const names = m[1] ? [m[1]] : m[2].split(",").map((x) => x.trim().split(/\s+as\s+/).pop()!);
    ...
    if (!base) continue;
    const hit = [".tsx", ".jsx", ".ts", ".js", ".vue", "/index.tsx", "/index.jsx", "/index.ts", "/index.js"].map((e) => base + e).find((f) => files.has(f));
...
function tagRe(names: string): RegExp {
  return new RegExp(`<(?:${names})\\b(?:[^>{}"']|"[^"]*"|'[^']*'|\\{(?:[^{}]|\\{[^{}]*\\})*\\})*>([\\s\\S]*?)<\\/(?:${names})>`, "g");
}

function pageFromFile(route: string, file: string, text: string, s?: SourceFiles): WebPage {
  const title =
  ...
  return {
    ...
    inputs: [...text.matchAll(/<(?:input|textarea|Input|Textarea)\b[^>]*?placeholder=["{]\s*["'`]?([^"'`}]+)["'`]?\s*}?/g)]
...
function routeFromFs(rel: string, dropExt = true): string {
  let r = rel;
  ...
  if (segs[segs.length - 1] === "index" || segs[segs.length - 1] === "page" || segs[segs.length - 1] === "+page") segs.pop();
  return "/" + segs.join("/");
}

function detectWeb(src: SourceFiles): MockOutput["web"] {
  const pages: WebPage[] = [];
  let framework = "";
  ...
  for (const p of src.paths) {
    ...
    } else if ((m = p.match(/^(?:.*\/)?(?:src\/)?pages\/(.+)\.(tsx|jsx|js|vue|astro|svelte|md|mdx)$/)) && !/^(api\/|_app|_document|_error)/.test(m[1])) {
  ...
  if (!pages.length) {
    for (const [file, text] of src.files) {
      ...
      const route = m[1].replace(/\/_[\w-]+/g, "").replace(/\$(\w+)/g, ":$1").replace(/\/$/, "") || "/";
    ...
    for (const [file, text] of src.files) {
      ...
      for (const m of text.matchAll(/path[=:]\s*\{?\s*["'`]([^"'`]*)["'`]/g)) {
        ...
        if (!pages.some((p) => p.route === route)) {
          ...
          const compFile = comp && [...src.files.keys()].find((f) => new RegExp(`(^|/)${comp}\\.(tsx|jsx|vue)$`).test(f));
          pages.push(compFile ? { ...pageFromFile(route, compFile, src.files.get(compFile)!, src), route } : { ...page, file });
        }
  ...
  // Static HTML sites
  if (!pages.length) {
    for (const p of src.paths.filter((x) => /\.html?$/.test(x) && !isTestPath(x) && x.split("/").length <= 3).slice(0, 10)) {
  ...
  const layout = [...src.files.entries()].find(([f]) => /(^|\/)(src\/)?app\/layout\.(tsx|jsx|js)$|(^|\/)pages\/_app\.(tsx|jsx)$|(^|\/)index\.html$|(^|\/)App\.(tsx|jsx|vue)$/.test(f));
  const siteTitle =
    (layout && (layout[1].match(/\btitle:\s*["'`]([^"'`$]{2,80})["'`]/)?.[1] ?? layout[1].match(/<title>([^<{]+)<\/title>/i)?.[1]?.trim())) ||
  ...
  pages.sort((a, b) => (a.route === "/" ? -1 : b.route === "/" ? 1 : a.route.split("/").length - b.route.split("/").length || a.route.localeCompare(b.route)));
  return { framework, siteTitle, pages: pages.slice(0, 12), theme: detectTheme(src), staticSite: framework === "Static HTML" };
}

// --- API endpoints ---------------------------------------------------------

const METHOD_PATTERNS: { re: RegExp; framework: string; method: (m: RegExpMatchArray) => string; path: (m: RegExpMatchArray) => string }[] = [
  {
  ...
  },
  {
    ...
    framework: "Flask",
    method: (m) => (m[2] ? m[2].replace(/["'\s]/g, "").split(",")[0].toUpperCase() : "GET"),
    path: (m) => m[1],
  },
  {
...
function maskComments(file: string, text: string): string {
  ...
  if (/\.(ts|tsx|js|jsx|mjs|cjs|go|rs|java|kt|php|cs)$/.test(file)) return text.replace(/\/\*[\s\S]*?\*\//g, blank).replace(/(^|[^:"'`\\])\/\/[^\n]*/g, blank);
  if (/\.rb$/.test(file)) return text.replace(/(^|\s)#[^\n]*/g, blank);
  return text;
}

/** From `start`, returns the balanced {...} / [...] / (...) expression, or null. */
function balanced(text: string, start: number, max = 700): string | null {
...
function responseNear(text: string, from: number): string | null {
  const window = text.slice(from, from + 2400);
  const next = window.slice(1).search(/\n\s*(?:@\w+\.(?:get|post|put|patch|delete|route)|\b(?:app|router|r)\.(?:get|post|put|patch|delete)\()/);
  const scope = next > 0 ? window.slice(0, next + 1) : window;
  const m = scope.match(/(?:\b(?:res|reply|c|ctx|response|w)\.(?:status\(\d+\)\.)?(?:json|send)\s*\(\s*|return\s+(?:jsonify\(\s*|JSONResponse\(\s*(?:content\s*=\s*)?|c\.json\(\s*|Response\.json\(\s*|NextResponse\.json\(\s*)?|ctx\.body\s*=\s*)([{[])/);
  if (!m) return null;
...
function bodyFieldsNear(text: string, from: number): string[] {
  const scope = text.slice(from, from + 1600);
  const fields = new Set<string>();
  for (const m of scope.matchAll(/(?:req|request|ctx\.request)\.body\.(\w+)/g)) fields.add(m[1]);
  for (const m of scope.matchAll(/(?:const|let|var)\s*\{([^}]+)\}\s*=\s*(?:await\s+)?(?:req\.body|request\.body|c\.req\.json\(\)|request\.json\(\)|await\s+req\.json\(\)|body)/g)) {
  ...
  for (const m of scope.matchAll(/request\.(?:json|form|get_json\(\))(?:\.get\(\s*|\[\s*)["'](\w+)["']/g)) fields.add(m[1]);
  return [...fields].slice(0, 8);
}

function detectPort(src: SourceFiles, only?: RegExp): number | null {
...
function detectApi(src: SourceFiles, port: number | null, defaultPort: number): MockOutput["api"] {
  const endpoints: Endpoint[] = [];
  const frameworks = new Map<string, number>();
  const seen = new Set<string>();
  const base = `http://localhost:${port ?? defaultPort}`;

  const push = (method: string, path: string, file: string, text: string, index: number, framework: string) => {
    ...
    const prefix = /\.py$/.test(file) ? text.match(/(?:APIRouter|Blueprint)\([^)]*?(?:url_)?prefix\s*=\s*["']([^"']+)["']/)?.[1] : undefined;
  ...
  for (const [file, raw] of src.files) {
    ...
    if (next) {
      for (const m of text.matchAll(/export\s+(?:async\s+)?(?:function|const)\s+(GET|POST|PUT|PATCH|DELETE)\b/g)) {
        push(m[1], routeFromFs(next[1], false).replace(/\/$/, "") || "/", file, text, m.index!, "Next.js route handlers");
      }
      continue;
    }
    ...
    if (pagesApi) {
      ...
      for (const method of methods.length ? [...new Set(methods)] : ["GET"]) push(method, routeFromFs(pagesApi[1]), file, text, 0, "Next.js API routes");
      continue;
    }
    if (/(^|\/)urls\.py$/.test(file)) {
      for (const m of text.matchAll(/\b(?:re_)?path\(\s*r?["']([^"']*)["']/g)) push("GET", "/" + m[1].replace(/^\^|\$$/g, ""), file, text, m.index!, "Django");
      continue;
    }
    if (/(^|\/)routes\.rb$/.test(file)) {
      for (const m of text.matchAll(/^\s*(get|post|put|patch|delete)\s+["']([^"']+)["']/gm)) push(m[1].toUpperCase(), m[2].startsWith("/") ? m[2] : "/" + m[2], file, text, m.index!, "Rails");
    ...
    for (const pat of METHOD_PATTERNS) {
      ...
      for (const m of text.matchAll(pat.re)) {
        ...
        if (fw === "Node.js HTTP") {
          fw = src.deps.has("hono") ? "Hono" : src.deps.has("fastify") ? "Fastify" : src.deps.has("koa") ? "Koa" : src.deps.has("elysia") ? "Elysia" : src.deps.has("express") ? "Express" : "Node.js HTTP";
        }
...
const methodRank = (m: string) => ["GET", "POST", "PUT", "PATCH", "DELETE"].indexOf(m);

// --- CLI -------------------------------------------------------------------

function detectCli(src: SourceFiles): MockOutput["cli"] {
  let binName: string | null = null;
  let version: string | null = null;
  for (const { json } of src.packageJsons) {
    if (json?.bin) {
      binName = typeof json.bin === "string" ? String(json.name ?? src.repoName).replace(/^@[^/]+\//, "") : Object.keys(json.bin)[0];
  ...
  const addCmd = (usage: string, description: string | null, file: string, text: string, index: number) => {
    if (commands.some((c) => c.usage === usage) || commands.length >= 24) return;
    commands.push({ usage, description, file, line: lineAt(text, index) });
  };
  const addOpt = (flag: string, description: string | null) => {
    ...
    if (/\.(ts|js|mjs|cjs)$/.test(file)) {
      ...
      if (/\.command\(|\.option\(/.test(text) && /commander|yargs|cac|sade|program\./.test(text)) {
        ...
        for (const m of text.matchAll(/\.command\(\s*["'`]([^"'`]+)["'`](?:\s*,\s*["'`]([^"'`]+)["'`])?/g)) {
          const desc = m[2] ?? text.slice(m.index!, m.index! + 300).match(/\.description\(\s*["'`]([^"'`]+)["'`]/)?.[1] ?? null;
          addCmd(m[1], desc, file, text, m.index!);
        }
        for (const m of text.matchAll(/\.option\(\s*["'`]([^"'`]+)["'`]\s*,\s*["'`]([^"'`]+)["'`]/g)) addOpt(m[1], m[2]);
        for (const m of text.matchAll(/\.option\(\s*["'](\w[\w-]*)["']\s*,\s*\{[^}]*?(?:describe|description|desc)\s*:\s*["'`]([^"'`]+)/g)) addOpt(`--${m[1]}`, m[2]);
      }
    } else if (/\.py$/.test(file)) {
      if (/import click|from click/.test(text)) {
        framework ||= "Click";
        for (const m of text.matchAll(/@(\w+)\.(?:command|group)\((?:\s*["']([\w-]+)["'])?[^)]*\)\s*\n(?:\s*@[^\n]+\n)*\s*(?:async\s+)?def\s+(\w+)\([^)]*\)[^:]*:\s*(?:["']{3}([\s\S]*?)["']{3})?/g)) {
          if (m[1] === "click" && !m[2]) continue; // the root command
          addCmd(m[2] ?? m[3].replace(/_/g, "-"), m[4]?.trim().split("\n")[0] ?? null, file, text, m.index!);
        }
        for (const m of text.matchAll(/@click\.option\(\s*["'](-{1,2}[\w-]+)["'](?:\s*,\s*["'](-{1,2}[\w-]+)["'])?[^)]*?help\s*=\s*["']([^"']+)["']/g)) addOpt([m[1], m[2]].filter(Boolean).join(", "), m[3]);
      }
      if (/import typer|from typer/.test(text)) {
        framework ||= "Typer";
        for (const m of text.matchAll(/@\w+\.command\((?:\s*["']([\w-]+)["'])?[^)]*\)\s*\n\s*(?:async\s+)?def\s+(\w+)\([^)]*\)[^:]*:\s*(?:["']{3}([\s\S]*?)["']{3})?/g)) {
          addCmd(m[1] ?? m[2].replace(/_/g, "-"), m[3]?.trim().split("\n")[0] ?? null, file, text, m.index!);
        }
      }
      if (/argparse/.test(text)) {
        framework ||= "argparse";
        for (const m of text.matchAll(/add_parser\(\s*["']([\w-]+)["'](?:[^)]*?help\s*=\s*["']([^"']+)["'])?/g)) addCmd(m[1], m[2] ?? null, file, text, m.index!);
        for (const m of text.matchAll(/add_argument\(\s*["']([^"']+)["'](?:\s*,\s*["'](-{1,2}[\w-]+)["'])?[^)]*?help\s*=\s*["']([^"']+)["']/g)) addOpt([m[1], m[2]].filter(Boolean).join(", "), m[3]);
      }
    } else if (/\.go$/.test(file)) {
      if (/cobra\.Command/.test(text)) {
        framework ||= "Cobra";
        for (const m of text.matchAll(/Use:\s*"([^"]+)"[\s\S]{0,300}?Short:\s*"([^"]+)"/g)) addCmd(m[1], m[2], file, text, m.index!);
        for (const m of text.matchAll(/Flags\(\)\.\w+?P?\(\s*(?:&[\w.]+\s*,\s*)?"([\w-]+)"\s*,\s*(?:"(\w)"\s*,\s*)?[^,]+,\s*"([^"]+)"\)/g)) addOpt(m[2] ? `-${m[2]}, --${m[1]}` : `--${m[1]}`, m[3]);
      }
...
function readmeUsage(readme: string | null, pkgName: string): { code: string; heading: string } | null {
...
function detectLibrary(src: SourceFiles, kinds: Set<ProjectKind>): MockOutput["library"] {
  ...
  const publishable = root && !root.private && root.name && (root.main || root.exports || root.module || root.types || (src.paths.includes("index.js") && (root.files || root.version)));
  if (publishable && !kinds.has("web")) {
    name = root.name ?? name;
    install = `npm install ${name}`;
    const entry = [...src.files.keys()].find((f) => /^(src|source|lib)?\/?index\.(ts|js|mjs)$/.test(f) || f === "index.ts" || f === "index.js");
    const entryText = entry ? src.files.get(entry)! : "";
    if (/export\s+default\b|module\.exports\s*=/.test(entryText)) exports.push("default");
    for (const m of entryText.matchAll(/(?:module\.)?exports\.(\w+)\s*=/g)) if (!exports.includes(m[1])) exports.push(m[1]);
    ...
    if (inner) for (const m of inner.matchAll(/(?:module\.)?exports\.(\w+)\s*=/g)) if (!exports.includes(m[1]) && exports.length < 24) exports.push(m[1]);
    if (entry) collect(entryText, /export\s+(?:default\s+)?(?:declare\s+)?(?:async\s+)?(?:function\*?|const|let|class|type|interface|enum)\s+([A-Za-z_$][\w$]*)|export\s*\{([^}]+)\}/g);
  }
  const py = [...src.files.entries()].find(([f]) => /(^|\/)pyproject\.toml$|(^|\/)setup\.py$/.test(f) && f.split("/").length === 1);
  if (!install && py) {
  ...
  if (!install && gomod && !src.paths.some((p) => /(^|\/)main\.go$/.test(p))) {
    ...
    install = `go get ${name}`;
    for (const [f, t] of src.files) if (/^[^/]+\.go$/.test(f) && !/_test\.go$/.test(f)) collect(t, /^func\s+([A-Z]\w*)/gm);
  }
...
export function detectKinds(src: SourceFiles, found: { web: boolean; api: boolean; cli: boolean; library: boolean }): ProjectKind[] {
  const k: ProjectKind[] = [];
  const d = src.deps;
  const has = (re: RegExp) => src.paths.some((p) => re.test(p));
  if (d.has("react-native") || d.has("expo") || d.has("flutter") || has(/^android\/app\/|^ios\/.*\.xcodeproj/)) k.push("mobile");
...
export function buildMock(src: SourceFiles): MockOutput {
  const web = detectWeb(src);
  const port = detectPort(src);
  const defaultPort = src.deps.has("next") || src.deps.has("express") ? 3000 : src.deps.has("vite") ? 5173 : src.deps.has("fastapi") || src.deps.has("django") ? 8000 : src.deps.has("flask") ? 5000 : 8080;
  ...
  const pre = new Set<ProjectKind>(web ? ["web"] : []);
