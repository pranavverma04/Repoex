...
export function getExplainJob(id) {
...
const running = new Map(); // fullName@branch -> jobId

export async function startExplain(input) {
  const ref = parseRepoInput(input);
  const meta = await fetchMeta(ref);
  if (meta.private) throw new UserFacingError("Private repositories aren't supported.", 403);
  if (meta.sizeKb / 1024 > 1500)
    throw new UserFacingError(
      `This repository is ${(meta.sizeKb / 1024).toFixed(0)} MB, which is too big to explain here.`,
      413,
    );
...
const STYLE =
  /(^|\/)((globals?|index|main|app|style|styles|theme|variables|tokens|base|site)\.(css|scss)|tailwind\.config\.\w+)$/;
const SKIP =
  /(^|\/)(node_modules|vendor|dist|build|out|\.next|target|coverage|__pycache__|\.venv|venv|third_party|external|deps)\//;

function pickFiles(paths) {
  const out = [];
  for (const p of paths) {
    ...
    if (
      MANIFEST.test(p) &&
      depth <= 4 &&
      !/(^|\/)(examples?|samples?|demos?|tests?|docs?|fixtures|templates?|benchmarks?)\//i.test(p)
    )
      out.push(p);
  ...
  const scored = paths
    ...
    .map((p) => {
      ...
      let score = -depth;
      if (
        /(route|router|api|server|app|main|index|cli|cmd|command|controller|view|page|layout|urls|handler|endpoint|__init__|__main__|lib\.rs|program)/i.test(
          p,
        )
      )
        score += 6;
      if (
        /(^|\/)(tests?|__tests__|spec|e2e|fixtures|examples?|docs?|benchmarks?|scripts)\//i.test(p) ||
        /\.(test|spec)\./.test(p)
      )
        score -= 8;
...
function languagesFromPaths(paths) {
  const map = {
    ts: "TypeScript",
    tsx: "TypeScript",
    js: "JavaScript",
...
async function runJob(job, ref, meta, branch, sha) {
  let co = null;
  const stage = async (key, fn, detail) => {
  ...
  try {
    ...
    const stack = await stage(
      ...
      (s) =>
        `${s.languages[0]?.name ?? "No code"}${s.groups.length ? ` · ${s.groups.reduce((n, g) => n + g.items.length, 0)} tools recognised` : ""}`,
    );
      ...
      () => {
        ...
        for (const { path, json } of packageJsons) {
          for (const field of ["main", "module"])
            if (typeof json[field] === "string") mains.push({ path, field, value: json[field] });
          if (typeof json.bin === "string") mains.push({ path, field: "bin", value: json.bin });
          else if (json.bin && typeof json.bin === "object")
            for (const v of Object.values(json.bin).slice(0, 2)) mains.push({ path, field: "bin", value: String(v) });
        }
    ...
    const mock = await stage(
      ...
      (m) =>
        [
          ...
          m.cli &&
            `${m.cli.commands.length || m.cli.options.length} command${(m.cli.commands.length || m.cli.options.length) === 1 ? "" : "s"}/flags`,
    ...
    const base = {
      ...
      repo: {
        ...
        openIssues:
          fetched.openPulls !== null
            ? Math.max(0, meta.openIssuesAndPulls - fetched.openPulls)
            : meta.openIssuesAndPulls,
      ...
      stack: {
        languages: stack.languages.slice(0, 12),
        groups: stack.groups,
        packageManagers: stack.packageManagers,
        otherDependencies: stack.otherDependencies,
      },
      structure: {
    ...
    const report = await stage(
      "write",
      async () => {
        const r = { ...base, overview: templateOverview(base, readme) };
    ...
    const row = db
      ...
      .get(meta.fullName.toLowerCase(), branch, sha, "{}");
    const full = { ...report, reportId: row.report_id };
    db.prepare(`UPDATE explain_reports SET report_json = ? WHERE report_id = ?`).run(
      JSON.stringify(full),
      row.report_id,
    );
    ...
    job.error =
      err instanceof UserFacingError
        ? message
        : `Something went wrong while explaining this repo: ${message.slice(0, 300)}`;
  } finally {
    const done = co;
...
export function getReport(id) {
  const row = db.prepare(`SELECT report_json FROM explain_reports WHERE report_id = ?`).get(id);
