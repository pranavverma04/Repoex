...
const BLOCKED_PORTS = new Set([Number(process.env.PORT ?? 8787), 3100]);
...
export async function localRequest(input: { port: unknown; method: unknown; path: unknown; body?: unknown }): Promise<LocalResponse> {
  ...
  if (!["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"].includes(method)) throw new UserFacingError("Unsupported method");
  const path = String(input.path ?? "/");
  if (!path.startsWith("/") || path.startsWith("//") || /[\s\\]/.test(path)) throw new UserFacingError("The path must start with /");
  const body = typeof input.body === "string" && input.body.trim() && !["GET", "HEAD"].includes(method) ? input.body : undefined;
