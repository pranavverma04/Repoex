...
import type { CloneJob, CloneLocation, RepoReport } from "@ara/shared";
...
export async function cloneLocations(): Promise<CloneLocation[]> {
...
async function resolveBase(input: string): Promise<string> {
  let raw = input.trim();
  ...
  if (real !== HOME && !real.startsWith(HOME + path.sep)) throw new UserFacingError("Pick a folder inside your home folder.");
  if (!(await fs.stat(real)).isDirectory()) throw new UserFacingError(`${tilde(base)} isn't a folder.`);
  const hidden = path.relative(HOME, real).split(path.sep).some((seg) => seg.startsWith("."));
  if (hidden || /^Library(\/|$)/.test(path.relative(HOME, real))) throw new UserFacingError("Pick a regular folder, not a hidden or system one.");
  return real;
}
...
function checkName(name: string): string {
  ...
  if (!/^[\w.@+-][\w .@+-]*$/.test(n) || n === "." || n === ".." || n.startsWith(".")) {
    throw new UserFacingError("Use letters, numbers, spaces, dots, dashes or underscores (no slashes, not starting with a dot).");
  }
  return n;
}
...
export async function checkTarget(base: string, name: string) {
...
export async function startClone(report: RepoReport, opts: { base: string; name: string; branch?: string; shallow?: boolean }): Promise<CloneJob> {
  const t = await checkTarget(opts.base, opts.name);
  if (t.exists) throw new UserFacingError(`${t.display} already exists. Pick another name${t.suggestion ? `, like ${t.suggestion}` : ""}.`, 409);
  ...
  const args = ["clone", "--progress", "--branch", branch, ...(opts.shallow ? ["--depth", "1"] : []), `https://github.com/${report.repo.owner}/${report.repo.name}.git`, t.path];
...
export async function openTerminal(id: string) {
  const job = jobs.get(id);
  if (!job || job.status !== "done") throw new UserFacingError("That clone isn't finished.", 404);
  await new Promise<void>((resolve, reject) => execFile("open", ["-a", "Terminal", job.dest], (err) => (err ? reject(err) : resolve())));
}
