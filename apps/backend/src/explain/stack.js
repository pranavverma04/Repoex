...
export function detectStack(input) {
  ...
  const add = (dep, version, source, dev = false) => {
  ...
  const addTool = (label, cat, source) => {
  ...
  for (const [p, text] of f) {
    ...
    if (/^requirements.*\.txt$/.test(base)) {
      ...
      for (const sec of text.matchAll(/\[(?:dev-|build-)?dependencies\]([\s\S]*?)(?=\n\[|$)/g)) {
        for (const m of sec[1].matchAll(/^([\w-]+)\s*=\s*(?:"([^"]+)"|\{[^}]*?version\s*=\s*"([^"]+)")?/gm))
          add(m[1], m[2] ?? m[3], p);
      }
      ...
      try {
        ...
        for (const field of ["require", "require-dev"])
          for (const [d, v] of Object.entries(j[field] ?? {})) add(d, v, p);
      } catch {}
  ...
  if (has(/\.ipynb$/))
    addTool(
      "Jupyter notebooks",
      "AI & ML",
      input.paths.find((p) => p.endsWith(".ipynb")),
    );

  const pms = [];
  const pm = (re, name) => has(re) && pms.push(name);
  ...
  if (
    !pms.some((x) => ["Poetry", "uv", "Pipenv"].includes(x)) &&
    has(/(^|\/)requirements.*\.txt$|(^|\/)pyproject\.toml$|(^|\/)setup\.py$/)
  )
    pms.push("pip");
