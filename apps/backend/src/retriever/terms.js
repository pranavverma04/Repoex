...
export function detectIntent(q) {
  ...
  if (/\bwhere\b.*\b(defined|declared|located|live|lives)\b|\bdefinition of\b|\bwhich file\b/.test(s))
    return "definition";
  return "explain";
}

const isIdentifierish = (t) => /[a-z][A-Z]|_|\d|^[A-Z][a-z]+[A-Z]/.test(t) || /^[A-Z][A-Za-z]+$/.test(t);
...
function stem(w) {
  ...
  for (const suf of ["ization", "ations", "ation", "ings", "ing", "ers", "er", "ies", "es", "ed", "s"]) {
    if (s.length - suf.length >= 4 && s.endsWith(suf))
      return suf === "ies" ? `${s.slice(0, -3)}y` : s.slice(0, -suf.length);
  }
  return s;
}
 ...
 */
export function reformulate(previous, question, attempt) {
  const out = new Set();
