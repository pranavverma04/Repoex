...
import type { SourceRef, SymbolKind } from "@ara/shared";
import { config } from "../config.js";
import { db } from "../db.js";
import type { Intent } from "./terms.js";
...
const KIND_BOOST: Record<string, number> = { class: 8, function: 6, method: 5, variable: -8, import: -40 };
const REASON_RANK: Reason[] = ["exact", "partial", "path", "content", "caller", "callee", "import"];
...
export function lexicalSearch(repoId: number, terms: string[]): Candidate[] {
  const scores = new Map<number, Candidate>();
  const bump = (sym: SymRow, score: number, reason: Reason) => {
  ...
  for (const term of terms) {
    if (term.includes(".")) {
      ...
      for (const s of qualifiedQ.all(repoId, parent, name) as SymRow[]) bump(s, 200, "exact");
      continue;
    }
    ...
    if (t.length >= 3) {
      ...
      for (const s of pathQ.all(repoId, `%${escapeLike(t)}%`) as SymRow[]) bump(s, 20, "path");
    }
    // Content mentions: only when the term is specific enough and names alone found little.
    if (t.length >= 4 && nameHits < 3) {
      for (const f of contentFilesQ.all(repoId, t) as { file_id: number; content: string }[]) {
        const lines = f.content.split("\n");
        const syms = fileSymsQ.all(f.file_id) as SymRow[];
  ...
  for (const c of scores.values()) {
    if (c.sym.parent && namedClasses.has(c.sym.parent.toLowerCase()) && (c.reason === "exact" || c.reason === "partial")) {
      c.score += 90;
    }
    // Test code mentions everything; rank it below the implementation unless tests were asked about.
    if (!asksAboutTests && /(^|\/)(tests?|__tests__|spec|test-d)\/|[._-](test|spec)\.\w+$|(^|\/)test_[^/]+$/.test(c.sym.path)) {
  ...
  for (const c of scores.values()) {
    ...
    const span = (x: Candidate) => x.sym.end_line - x.sym.start_line;
...
export function expandGraph(seeds: Candidate[], intent: Intent): Candidate[] {
  const { maxHops, maxContextSymbols } = config.retrieval;
  const hops = intent === "callers" || intent === "callees" ? maxHops : 1;
  const out = new Map<number, Candidate>(seeds.map((c) => [c.sym.symbol_id, c]));
  ...
  for (let hop = 1; hop <= hops && out.size < maxContextSymbols; hop++) {
    const nextFrontier: Candidate[] = [];
    for (const c of frontier) {
      const decay = 0.5 ** hop;
      const wantCallers = intent !== "callees";
      const wantCallees = intent !== "callers" || hop === 1;
      const neighbours: [SymRow & { edge_type: string }, Reason][] = [];
      if (wantCallers) for (const r of callersQ.all(c.sym.symbol_id) as (SymRow & { edge_type: string })[]) {
        neighbours.push([r, r.edge_type === "imports" ? "import" : "caller"]);
      }
      if (wantCallees) for (const r of calleesQ.all(c.sym.symbol_id) as (SymRow & { edge_type: string })[]) {
      ...
      for (const [sym, reason] of neighbours) {
        ...
        const boost = (intent === "callers" && reason === "caller") || (intent === "callees" && reason === "callee") ? 1.5 : 1;
        const cand: Candidate = { sym, score: Math.round(c.score * decay * boost), reason, via: c.sym.name };
...
export function assembleContext(cands: Candidate[]): SourceRef[] {
  ...
  for (const c of cands) {
    ...
    if (!lines) {
      lines = ((contentQ.get(c.sym.file_id) as { content: string }).content ?? "").split("\n");
...
export function neighbours(symbolId: number) {
  const q = (sql: string) => db.prepare(sql).all(symbolId) as { name: string; parent: string | null; path: string }[];
  const fmt = (r: { name: string; parent: string | null }) => (r.parent ? `${r.parent}.${r.name}` : r.name);
  return {
    calls: q(
