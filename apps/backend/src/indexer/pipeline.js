...
export async function runIndexJob(jobId, repoId, type) {
  const started = Date.now();
  const repo = db.prepare(`SELECT repo_id, url FROM repos WHERE repo_id = ?`).get(repoId);
  ...
  try {
    ...
    const stored = new Map(
      db
        .prepare(`SELECT file_id, path, hash FROM files WHERE repo_id = ?`)
        .all(repoId)
        .map((r) => [r.path, r]),
    );
    ...
    const symbolCount = db.prepare(`SELECT COUNT(*) AS n FROM symbols WHERE repo_id = ?`).get(repoId).n;
