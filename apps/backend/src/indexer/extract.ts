...
import type { SymbolKind } from "@ara/shared";
import { parserFor, type Language } from "./languages.js";
...
export const symbolKey = (name: string, startLine: number) => `${name}:${startLine}`;

export interface CallRef {
  from: string; // symbolKey of the enclosing definition
  callee: string;
  qualifier: string | null; // `obj` in obj.method(), `self`/`this` for own-class calls
}

export interface ImportRef {
  symbol: string; // symbolKey of the import symbol
  module: string;
  imported: string; // name as exported by the target module ("default"/"*" for those forms)
}
...
function leadingComment(n: Node): string | null {
...
function pythonDocstring(def: Node): string | null {
  const body = def.childForFieldName("body");
  const first = body?.namedChild(0);
  if (first?.type === "expression_statement" && first.namedChild(0)?.type === "string") {
    return first.namedChild(0)!.text.replace(/^[rbuf]*("""|'''|"|')|("""|'''|"|')$/gi, "").replace(/\s+/g, " ").trim().slice(0, 400);
  }
  return null;
}

export async function extract(lang: Language, source: string): Promise<ExtractResult> {
  const parser = await parserFor(lang);
  const tree = parser.parse(source);
  const symbols: ParsedSymbol[] = [];
  const refs: FileRefs = { calls: [], imports: [] };
  try {
    const add = (nameNode: Node | null | string, node: Node, kind: SymbolKind, ctx: Ctx, doc?: string | null) => {
    ...
    const addCall = (callee: string | undefined, qualifier: string | null, ctx: Ctx) => {
      if (callee && ctx.enclosing) refs.calls.push({ from: ctx.enclosing, callee, qualifier });
    };
    const addImport = (local: string, imported: string, module: string, node: Node, ctx: Ctx) => {
...
function isTransparent(n: Node) {
  return /^(program|module|source_file|export_statement|decorated_definition|expression_statement|lexical_declaration|variable_declaration|class_body|block|declaration_list)$/.test(
    n.type,
  ) || n.parent === null;
}

interface Emit {
  add: (name: Node | null | string, node: Node, kind: SymbolKind, ctx: Ctx, doc?: string | null) => string | null;
  addCall: (callee: string | undefined, qualifier: string | null, ctx: Ctx) => void;
  addImport: (local: string, imported: string, module: string, node: Node, ctx: Ctx) => void;
}

/** Each handler inspects one node; returning a Ctx means "children run inside this definition". */
type Handler = (n: Node, ctx: Ctx, e: Emit) => Ctx | void;

const field = (n: Node, f: string) => n.childForFieldName(f);

const jsLike: Handler = (n, ctx, e) => {
  switch (n.type) {
    ...
    case "call_expression": {
      ...
      if (fn?.type === "identifier") e.addCall(fn.text, null, ctx);
        ...
        e.addCall(field(fn, "property")?.text, obj?.type === "this" ? "this" : obj?.type === "identifier" ? obj.text : null, ctx);
      }
      return;
    }
    case "new_expression": {
...
const rust: Handler = (n, ctx, e) => {
  switch (n.type) {
    ...
    case "call_expression": {
      ...
      } else if (fn?.type === "scoped_identifier") e.addCall(field(fn, "name")?.text, field(fn, "path")?.text ?? null, ctx);
      return;
    }
  }
};

const java: Handler = (n, ctx, e) => {
  switch (n.type) {
    case "class_declaration":
    case "interface_declaration":
    case "enum_declaration":
    ...
    case "method_invocation": {
      ...
      e.addCall(field(n, "name")?.text, obj?.type === "this" ? "this" : obj?.type === "identifier" ? obj.text : null, ctx);
      return;
    }
...
const handlers: Record<Language, Handler> = {
  javascript: jsLike,
  typescript: jsLike,
  tsx: jsLike,
  python,
