...
import { symbolKey } from "./extract.js";
...
function resolveModule(from, spec, byPath, allPaths) {
...
function looksLikeInstanceOf(variable, className) {
...
export function resolveEdges(repoId) {
  const files = db.prepare(`SELECT file_id, path, language, refs_json FROM files WHERE repo_id = ?`).all(repoId);
  const syms = db
  ...
  const byKey = new Map(); // `${fileId}|name:line`
  ...
  for (const file of files) {
    const refs = JSON.parse(file.refs_json);
    const importedFiles = new Set();
    const importTargets = new Map(); // local name -> resolved definition
    const namespaces = new Map(); // `import * as x` / `import pkg` -> files
    ...
    for (const call of refs.calls) {
      ...
      if (q === "this" || q === "self") {
        ...
        const importedClasses = new Set(
          [...importTargets.values()].filter((t) => t.kind === "class").map((t) => t.name),
        );
  ...
  write();
  return db.prepare(`SELECT COUNT(*) AS n FROM edges WHERE repo_id = ?`).get(repoId).n;
}
