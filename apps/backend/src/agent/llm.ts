...
import type { Intent } from "../retriever/terms.js";

type Provider = "anthropic" | "groq" | "gemini" | "extractive";
...
function renderContext(question: string, sources: SourceRef[]) {
...
async function openAiCompatible(provider: "groq" | "gemini", prompt: string): Promise<string> {
  const cfg =
    provider === "groq"
...
export function extractiveAnswer(question: string, sources: SourceRef[], intent: Intent): string {
  if (!sources.length) {
  ...
  if (intent === "callers") {
    out.push(rel.calledBy.length ? `**Called by:** ${list(rel.calledBy)}` : "No callers were found in the indexed call graph.");
  } else if (intent === "callees") {
    out.push(rel.calls.length ? `**Calls:** ${list(rel.calls)}` : "It makes no calls that resolve to other symbols in this repository.");
  } else {
...
export async function synthesize(
  ...
  sources: SourceRef[],
  intent: Intent,
): Promise<{ answer: string; synthesizer: string }> {
