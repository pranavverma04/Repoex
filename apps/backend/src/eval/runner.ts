...
import type { EvalResult, EvalRunSummary } from "@ara/shared";
...
interface DatasetRepo {
  repoUrl: string;
  questions: { question: string; expectedSymbols: string[]; expectedAnswer: string }[];
}

const here = path.dirname(fileURLToPath(import.meta.url));
const loadDataset = (): DatasetRepo[] => JSON.parse(fs.readFileSync(path.join(here, "dataset.json"), "utf8"));

const WORD_STOP = new Set(["that", "this", "with", "from", "into", "then", "than", "they", "their", "which", "when", "uses", "used"]);

/** Share of the expected answer's key words that show up in the produced answer. */
function answerScore(expected: string, answer: string) {
  const words = [...new Set(expected.toLowerCase().match(/[a-z_][a-z0-9_]{3,}/g) ?? [])].filter((w) => !WORD_STOP.has(w));
  if (!words.length) return 1;
  const a = answer.toLowerCase();
  return words.filter((w) => a.includes(w)).length / words.length;
}

async function ensureIndexed(repoUrl: string): Promise<number> {
  const url = parseRepoUrl(repoUrl).url;
  let row = db.prepare(`SELECT repo_id, status FROM repos WHERE url = ?`).get(url) as { repo_id: number; status: string } | undefined;
  if (!row || row.status === "failed") {
    await submitRepo(repoUrl);
  }
  // Works both inside the API process (worker already running) and from the CLI.
  await drainQueue();
  row = db.prepare(`SELECT repo_id, status FROM repos WHERE url = ?`).get(url) as { repo_id: number; status: string };
  if (row.status !== "ready") throw new Error(`Could not index ${repoUrl}`);
  return row.repo_id;
}

export async function runEval(): Promise<EvalRunSummary[]> {
  ...
  for (const ds of loadDataset()) {
    ...
    for (const q of ds.questions) {
      const { question_id } = db
        ...
        .get(repoId, q.question, JSON.stringify(q.expectedSymbols), q.expectedAnswer) as { question_id: number };
      ...
      db.prepare(
        `INSERT INTO eval_results (question_id, run_id, retrieval_pass, answer_score, passed, latency_ms, retrieved)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).run(question_id, runId, +retrievalPass, result.answerScore, +result.passed, result.latencyMs, JSON.stringify(retrieved));
    }
...
function summarize(runId: string, repoUrl: string, results: EvalResult[]): EvalRunSummary {
  return {
    runId,
    repoUrl,
...
export function latestEvalRun(): EvalRunSummary[] {
  const last = db.prepare(`SELECT run_id FROM eval_results ORDER BY result_id DESC LIMIT 1`).get() as { run_id: string } | undefined;
  if (!last) return [];
  const rows = db
    .prepare(
    ...
    .all(last.run_id) as any[];
  const byRepo = new Map<string, EvalResult[]>();
  for (const r of rows) {
